...
import eCommerce.services.TunicsServiceRepo;

@Controller
public class TunicsController {
	
	@Autowired(required = true)
	private TunicsRepo turepo;
	
	@Autowired(required = true)
	private TunicsServiceRepo tunicsServiceRepo;

	@GetMapping("/tunics")
	public String mainAdmin()
	{   
		return "AdminTunicsFillingForm";
	}
	
	
	@PostMapping("/addtunics")
	...
	{
		try {
			tunicsServiceRepo.saveTunics(id, title, price_dis, price_ori, off, style_name, color, category, pattern, image1, image2, image3, image4, image5);
		} catch (IOException e) {
	...
	@GetMapping("/getTunics")
	public String getAllTunics(Model model)
	{
		List<Tunics_table> listtunics = turepo.findAll();
		model.addAttribute("listtunics", listtunics);
		return "gettunicsproducts";
	}
