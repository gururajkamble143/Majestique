...
import eCommerce.services.NewarrivalServiceRepo;
...
public class NewArrivalController {
	...
	@Autowired(required = true)
	private NewarrivalServiceRepo newarrivalServiceRepo;

	@GetMapping("/newarrival")
	public String mainAdmin()
	{   
		return "AdminNewArrivalFillingForm";
	}
	
	
	@PostMapping("/addnewarrival")
	...
	{
		try {
			newarrivalServiceRepo.saveNewarrival(id, title, price_dis, price_ori, off, style_name, color, category, pattern, image1, image2, image3, image4, image5);
		} catch (IOException e) {
	...
	{
		List<Newarrival_table> listnewarrival = narepo.findAll();
		model.addAttribute("listnewarrival", listnewarrival);
