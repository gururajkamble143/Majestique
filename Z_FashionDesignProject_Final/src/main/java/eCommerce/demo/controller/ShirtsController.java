...
import eCommerce.services.ShirtsServiceRepo;
...
public class ShirtsController {
	...
	@Autowired(required = true)
	private ShirtsServiceRepo shirtServiceRepo;
	
	@GetMapping("/shirts")
	public String mainAdmin()
	{   
		return "AdminShirtsFillingForm";
	}
	
	
	@PostMapping("/addshirts")
	...
	{
		try {
			shirtServiceRepo.saveShirts(id, title, price_dis, price_ori, off, style_name, color, category, pattern, image1, image2, image3, image4, image5);
		} catch (IOException e) {
	...
	public String getAllShirts(Model model)
	{
		List<Shirts_table> listshirts = srepo.findAll();
		model.addAttribute("listshirts", listshirts);
		return "getshirtsproducts";
	}
